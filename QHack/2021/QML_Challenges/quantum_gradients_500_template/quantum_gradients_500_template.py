...
def natural_gradient(params):
    ...
    # First, we calculate the metric tensor (Fubini-Study metric)
    @qml.qnode(dev)
    def circuit(params, ei, ej):
        variational_circuit(params + (ei+ej)*np.pi/2)        
        qml.adjoint(variational_circuit)(params)
    ...
    for i in range(6):
        for j in range(1+i):
            F[i,j] = 1/8 * (-circuit(params, basis_vectors[i], basis_vectors[j])[0]
                            +circuit(params, basis_vectors[i], -1*basis_vectors[j])[0]
                            +circuit(params, -1*basis_vectors[i], basis_vectors[j])[0]
                            -circuit(params, -1*basis_vectors[i], -1*basis_vectors[j])[0])
            F[j,i] = F[i,j]
                
    # Now we get the gradients of f
    
    shift = np.pi/2
    gradient = np.zeros(6)
    for i in range(6):
        gradient[i] = (qnode(params + basis_vectors[i]*shift) - qnode(params - basis_vectors[i]*shift))/2
    # QHACK #
    
    F_inv = np.linalg.inv(F)
    
    natural_grad = F_inv @ gradient
